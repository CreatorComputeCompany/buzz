...
7. **Orca session surface: mount Orca Web's real renderer, unchanged** — this is a hard constraint, not a preference. Mount Orca Web's actual entrypoint in Buzz's light DOM at the Chat session view (`desktop/src/features/chats/orca/OrcaSessionView.tsx`). The ONLY permitted fork point is the auth/runtime bootstrap: replace Orca's own handshake with the Buzz runtime ticket from `pairing.ts`. Never use Shadow DOM, never use an iframe, never rebuild a stripped copy of Orca's session UI, and never add portal or keyboard bridge shims. A previous attempt did exactly that and failed (Radix portals escape shadow roots; xterm.js keyboard/IME input never reached the PTY); acceptance is `desktop/scripts/verify-hosted-orca-chat.mjs`. Keep Orca as an upstream dependency so its updates can be pulled without re-porting UI code.
